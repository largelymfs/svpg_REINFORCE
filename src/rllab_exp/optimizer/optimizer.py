...
#     Created By          :     yang
#     Creation Date       :     [2017-03-18 23:49]
#     Last Modified       :     [2017-04-06 19:15]
#     Description         :      
#################################################################################
...
class MyFirstOrderOptimizer(Serializable):
    """
    ...
    def update_opt(self, loss, target, inputs, extra_inputs=None, gradients=None, **kwargs):
        ...
        if gradients is None:
            ...
            gradients_flatten = TT.concatenate([TT.flatten(i) for i in gradients])
    ...
    def optimize_gen(self, inputs, extra_inputs=None, callback=None, yield_itr=None):
        ...
        # by yang
        # we just leave the code here since the epoch is 1 in the settings
        itr = 0
        for epoch in pyprind.prog_bar(list(range(self._max_epochs))):
            for batch in dataset.iterate(update=True):
                #f_opt(*batch)
                self.gradient = f_grad(*batch)
